...
namespace MetricsManagerClient.Jobs
{
    class NetworkMetricJob : IJob
    {
        ...
        {
            AllNetworkMetricsResponse networkMetrics = new AllNetworkMetricsResponse();
            try
            {
                using (HttpClient httpClient = new HttpClient())
                {
                    using (HttpResponseMessage response = await httpClient.GetAsync(requestAddress))
                    {
                        using (HttpContent content = response.Content)
                        {
                            ...
                            networkMetrics = JsonConvert.DeserializeObject<AllNetworkMetricsResponse>(data);
